...
def run(params):
    ...
    imported_problem = importlib.import_module(
        "problems." + params.problemModule)
    bc = imported_problem.BoundaryConditions()

    # Solve the multires problem
    (x, nelx, nely, results, aggregated_hist) = multires.multires(
        params.nelx, params.nely, params, bc)

    # Convert to RGB
    x_rgb = 1.0 - \
        numpy.reshape(numpy.repeat(x.reshape((nelx, nely)),
                                   3, axis=1), (nelx, nely, 3))
...
def plot_history(args, outputname):
    ...
    for level in aggregated_hist.keys():
        ...
        x1.append(
            range(temp1, temp1 + len(aggregated_hist[level]['Compliance'])))
        temp1 = temp1 + len(aggregated_hist[level]['Compliance'])
        y2.append(aggregated_hist[level]['Appearance'])
        x2.append(
            range(temp2, temp2 + len(aggregated_hist[level]['Appearance'])))
...
def output_name(params):
    ...
    if str(params.problemType) == "ProblemType.AppearanceWithMaxComplianceAndSymmetry":
        name += 'AWMC_symmetric_'
        name += params.exemplarPath[7:len(params.exemplarPath)-4]+'_'
        name += 'eds' + str(params.exemplarDownsampling) +'_'

    if str(params.problemType) == "ProblemType.ComplianceWithSymmetry":
        name += 'Conly_symmetric'+'_'

    if str(params.problemType) == "ProblemType.AppearanceWithMaxCompliance":
        name += 'AWMC_'
        name += params.exemplarPath[7:len(params.exemplarPath)-4]+'_'
        name += 'eds' + str(params.exemplarDownsampling) +'_'
...
def main(args):
    ...
    start = time.time()
    (x_rgb_out, nelx_out, nely_out, results_out,
     aggregated_hist) = run(input_params)
    print(results_out, nelx_out, nely_out)
    history_name = 'output/' + \
        os.path.split(os.path.splitext(args.input_json)[0])[
            1] + "_histories.json"
    with open(history_name, 'w') as json_file:
        json_file.write(json.dumps(aggregated_hist))
    outputname = output_name(input_params)
    scipy.misc.toimage(x_rgb_out.T, cmin=0.0,
                       cmax=1.0).save(outputname + '.png')
    end = time.time()
    print('Ellapsed time : ' + str(int((end - start)) / 60) +
          'min ' + str(int((end - start)) % 60) + 'sec')
